...
public class main {
   ...
   public static boolean ficheroEntradaOk=false;        // Boolean que indica si el fichero de entrada estÃ¡ ok   
   ...
   public static void main(String[] args) throws IOException {  
       ...
       for (int i = 0; i<args.length; i++){
           ...
           if (argumento.equals("-t") ) { 
               ...
               System.out.println("Introduce el nÃºmero de activos:");
       ...
       // If-else que en funciÃ³n de los atributos boolean hace la mayor parte de lo que aparece por pantalla en el programa
       if(imprimirAyuda) {
           ...
           if (!operaciones.comprobarFicheroEntrada(ficheroOk)) { 
               if(operaciones.error == 1){System.out.println("ERROR: En la primera fila no has introducido un nÃºmero.");
               } else if(operaciones.error == 2){System.out.println("ERROR: En la segunda fila no has introducido un vector de nÃºmeros.");
               } else if(operaciones.error == 3){System.out.println("ERROR: El nÃºmero de la primera fila no coincide con la talla del vector de la segunda fila.");
               } else { System.out.println("ERROR DESCONOCIDO.");}
           } else {
               if(!mostrarPorPantalla){ 
                   System.out.println("NÂº de activos: : " + operaciones.nActivos);
