...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int entrada = 0; entrada < args.length; entrada++) {
            ...
            int jugadores = Integer.valueOf(args[0]);
            try{
                Integer.parseInt(args[entrada]);
                return true;
            } catch ( NumberFormatException argumentoAyuda){
                return false;
            }
            if ( argumento.length() == 1 && argumento.equals("-h")) {
                System.out.println("Vemos la ayuda del programa ... CARGANDO...\n\n");
                verAyuda = true;
            }
            if (  argumento.length() == 2 && argumento.equals("-t")) {
